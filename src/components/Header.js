      ...
      <div className=" fixed flex justify-between min-h-[100px] items-center w-[98%] mx-auto z-10  ">
        ...
        <ul className="flex gap-5 items-center">
          {menus.map((menu) => (
            <Link
              to={menu.url}
              className="focus:underline focus:bg-opacity-80 hover:bg-opacity-80   bg-opacity-40 bg-[#d6d4d1] tracking-[.1825rem] font-medium text-[0.6rem] px-3 h-7 text-center rounded-md flex items-center cursor-pointer "
            >
              <li> {menu.name}</li>
            </Link>
          ))}
